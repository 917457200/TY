    ...
    if (Data != null && Data.length > 0) {
        ...
        for (i = 0; i < rows; i++) {
            ...
            for (j = 0; j < values.length; j++) {
                ...
                } else if (values[j] == "SwitchDate") {
                    //设备开关机时间
                    if (Data[i][values[j]].toString() != "") {
                        result = result + "<td  align=\"center\">" + Data[i][values[j]] + "</td>";
                    }
                    else {
                        result = result + "<td align=\"center\"> 6:30 - 21:30 </td>";
                    }

                } else if (values[j] == "SwitchDatelong") {
                    //设备使用时长
                    var SwitchDatelong = SwitchDate(Data[i]);
                    result = result + SwitchDatelong;
                } else if (values[j] == "UseIngLv") {
                    //设备使用率
                } else {
                    //普通列数据加载
...
function Transfer(Code,name) {
    ...
    kuan = kuan + " <input type=\"button\" id=\"popup_ok\" class=\"widgetcontent\" value=\"确定\" onclick=\"Delete('" + Code + "')\">";
